...
user_voc_id = []
user_emb_freq = []
for user, user_list in user2users.items():
    #if len(user_list) < 3 or len(user_list)> 20:
    if len(user_list) < 2:
        user_voc_id.append(user)
        user_emb_freq.append(user_emb[user])

print("filter user num:", len(user_voc_id))
Y = cdist(user_emb_freq, user_emb_freq, 'minkowski', p=2.)
