...
'''
class SEPTV2_yelp_add_other_sl_v2_kl_mlp_add(SocialRecommender, GraphRecommender):
    def __init__(self, conf, trainingSet=None, testSet=None, relation=None, fold='[1]'):
        GraphRecommender.__init__(self, conf=conf, trainingSet=trainingSet, testSet=testSet, fold=fold)
        SocialRecommender.__init__(self, conf=conf, trainingSet=trainingSet, testSet=testSet, relation=relation,fold=fold)

    def readConfiguration(self):
        super(SEPTV2_yelp_add_other_sl_v2_kl_mlp_add, self).readConfiguration()
        args = config.OptionConf(self.config['SEPTV2_yelp_add_other_sl_v2_kl_mlp_add'])
        ...
        self.ppr_flag = int(args['-ppr_flag']) 
        # pdb.set_trace()
    ...
    def hierarchical_self_supervision_new(self,em,adj, add_global=True, add_local=True):
        def row_shuffle(embedding):
            return tf.gather(embedding, tf.random.shuffle(tf.range(tf.shape(embedding)[0])))
        def row_column_shuffle(embedding):
            corrupted_embedding = tf.transpose(tf.gather(tf.transpose(embedding), tf.random.shuffle(tf.range(tf.shape(tf.transpose(embedding))[0])))) #(user, dim)
    ...
    def hierarchical_self_supervision_new_v2(self,em, adj_list, add_global=True, add_local=True):
        ...
        for adj_one in adj_list:
            ...
            if self.dataset_name == "douban_book":
                edge_embeddings_list.append(tf.math.l2_normalize(tf.matmul(adj_one,user_embeddings), axis=1))
            else:
    ...
    def cal_ppr_social_mat(self, weight=0.15, type_enc="ppr", bidirectional=False):
        ...
        social_mat = normalization(social_mat) #normal origin social mat;
    ...
    def initModel(self):
        super(SEPTV2_yelp_add_other_sl_v2_kl_mlp_add, self).initModel()
        ...
        if self.ppr_rate != 0.0:
            ...
            for sub_rec_user_embs in rec_user_embeddings_split:
                ...
                # self.rec_user_user_sim_list.append(tf.nn.sigmoid(tf.matmul(sub_rec_user_embs, tf.transpose(self.rec_user_embeddings, perm=[1, 0]))))
        # pdb.set_trace()
        ...
        if self.social_ppr_cluster_w !=0.0 or self.inter_cl_w != 0.0:
            social_ppr_mat, social_ppr_sp_mat = self.cal_ppr_social_mat()
            self.social_ppr_sub_tensor_list_ppr_v2 = self._split_A_hat(social_ppr_mat)
        ...
        # self.weights['sgating_bias'] = tf.Variable(initializer([1, self.emb_size]), name='sg_W_b_1')
        # self.weights = {}
        # initializer = tf.contrib.layers.xavier_initializer()
        # self.weights['sgating_bias'] = tf.Variable(initializer([1, self.emb_size]), name='sg_W_b_1')
        # self.weights['sgating'] = tf.Variable(initializer([self.emb_size, self.emb_size]), name='sg_W_1')

        # def self_supervised_gating(em):
        #     return tf.multiply(em,tf.nn.sigmoid(tf.matmul(em, self.weights['sgating'])+self.weights['sgating_bias']))

        if self.inter_cl_w != 0.0:
            # social_ppr_sp_mat_sp = sp.csr_matrix(social_ppr_mat)
            # social_ppr_sp_mat_true = self._convert_sp_mat_to_sp_tensor(social_ppr_sp_mat_true) # original social graph;
            # if self.ppr_rate == 0.0:
            #     self.social_ppr_sub_tensor_list = self._split_A_hat(social_ppr_mat)
            
            social_ppr_sub_tensor_list_v2 = self.social_ppr_sub_tensor_list_ppr_v2
            # social_ppr_sub_tensor_total = tf.concat(social_ppr_sub_tensor_list_v2, axis=0)
            # social_ppr_sub_tensor_list_v2 = self._split_A_hat_v2(social_ppr_sp_mat_sp) # list of sparse tensors


            # social_ppr_sp_mat_true = tf.convert_to_tensor(social_ppr_mat, dtype=tf.float32)
            ...
            # self.sl_loss = self.hierarchical_self_supervision(self.rec_user_embeddings, social_ppr_sp_mat_true)
            # no split
            # self.sl_loss = self.hierarchical_self_supervision_new(self.rec_user_embeddings, social_ppr_sub_tensor_total, add_local=True, add_global=True)
    ...
    def trainModel(self):
        ...
        if self.ppr_rate != 0.0:
            ...
            for ppr_score, user_sim in zip(self.social_ppr_sub_tensor_list, self.rec_user_user_sim_list):
                if self.ppr_flag == 1:
                    ...
                    loss_list.append(tf.reduce_sum(tf.losses.sigmoid_cross_entropy(ppr_score, user_sim, reduction=tf.losses.Reduction.NONE), axis=-1))
        ...
        if self.inter_cl_w != 0.0:
            loss += self.inter_cl_w * self.sl_loss
        ...
        for epoch in range(self.maxEpoch):
            ...
            # if epoch > -1:
                ...
                for n, batch in enumerate(self.next_batch_pairwise()):
                    ...
                    # _, opt_l, l1, l2, l3, l4, l5 = self.sess.run([v2_op, loss, rec_loss, self.social_ppr_loss, self.social_ppr_cluster_loss, self.sl_loss, reg_loss],feed_dict=feed_dict)
                    ...
                    # print(self.foldInfo, 'training:', epoch + 1, 'batch', n, 'total loss:', opt_l, 'rec loss:', l1, 'reg loss:', l5, 'other_cl_loss:', l4)
