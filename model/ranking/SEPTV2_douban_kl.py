...
'''
class SEPTV2_douban_kl(SocialRecommender, GraphRecommender):
    def __init__(self, conf, trainingSet=None, testSet=None, relation=None, fold='[1]'):
        GraphRecommender.__init__(self, conf=conf, trainingSet=trainingSet, testSet=testSet, fold=fold)
        SocialRecommender.__init__(self, conf=conf, trainingSet=trainingSet, testSet=testSet, relation=relation,fold=fold)

    def readConfiguration(self):
        super(SEPTV2_douban_kl, self).readConfiguration()
        args = config.OptionConf(self.config['SEPTV2_douban_kl'])
    ...
    def initModel(self):
        super(SEPTV2_douban_kl, self).initModel()
    ...
    def trainModel(self):
        ...
        for epoch in range(self.maxEpoch):
            ...
            # if epoch > -1:
                ...
                for n, batch in enumerate(self.next_batch_pairwise()):
                    ...
                    # _, opt_l, l1, kl_loss = self.sess.run([v2_op, loss, rec_loss, self.kl_loss], feed_dict=feed_dict)
                ...
                for n, batch in enumerate(self.next_batch_pairwise()):
                    ...
                    #                       feed_dict=feed_dict)
                    # _, l1, reg_l, kl_loss = self.sess.run([v1_op, rec_loss, reg_loss, self.kl_loss],
                    #                       feed_dict=feed_dict)
        ...
        # self.U, self.V: numpy
        # # save
        # np.save('./exp/lastfm/user_emb', self.U)
        # np.save('./exp/lastfm/item_emb', self.V)
        # with open('./exp/lastfm/id2user.pickle', 'wb') as handle:
        #     pickle.dump(self.data.id2user, handle, protocol=pickle.HIGHEST_PROTOCOL)
        # with open('./exp/lastfm/id2item.pickle', 'wb') as handle:
