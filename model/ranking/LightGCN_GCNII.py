...
LightGCN model
"""
class LightGCN_GCNII(GraphRecommender):
    def __init__(self,conf,trainingSet=None,testSet=None,fold='[1]'):
        super(LightGCN_GCNII, self).__init__(conf,trainingSet,testSet,fold)
        args = OptionConf(self.config['LightGCN'])
        self.n_layers = int(args['-n_layer'])
        # pdb.set_trace()

    def initModel(self):
        super(LightGCN_GCNII, self).initModel()
        #SEPT对齐, 修正初始化.
        ...
        ego_embedding_init = ego_embeddings
        norm_adj = self.create_joint_sparse_adj_tensor()
        all_embeddings = [ego_embeddings]
        initializer = tf.contrib.layers.xavier_initializer()

        for k in range(self.n_layers):
            weight_layer = tf.Variable(initializer([self.emb_size * 2, self.emb_size]), name='attention_m1%d' % k)
            # GCNII
            self.variant = True
            lamda, alpha = 0.5, 0.5
            theta = math.log(lamda / (k + 1))
            ego_embeddings = tf.sparse_tensor_dense_matmul(norm_adj, ego_embeddings)
            # add trick
            if self.variant:
                support = tf.concat([ego_embeddings, ego_embedding_init], axis=1)
                r = (1 - alpha) * ego_embeddings + alpha * ego_embedding_init
            else:
                support = (1 - alpha) * ego_embeddings + alpha * ego_embedding_init
                r = support
            output = theta * tf.matmul(support, weight_layer) + (1 - theta) * r
            ego_embeddings = output
