...
'''
class SEPT_social(SocialRecommender, GraphRecommender):
    def __init__(self, conf, trainingSet=None, testSet=None, relation=None, fold='[1]'):
        GraphRecommender.__init__(self, conf=conf, trainingSet=trainingSet, testSet=testSet, fold=fold)
        SocialRecommender.__init__(self, conf=conf, trainingSet=trainingSet, testSet=testSet, relation=relation,fold=fold)

    def readConfiguration(self):
        super(SEPT_social, self).readConfiguration()
        args = config.OptionConf(self.config['SEPT_social'])
    ...
    def cal_ppr_social_mat(self, weight=0.15):
        ...
        # c = 0.15
        eigen_adj = weight * inv((sp.eye(adj.shape[0]) - (1 - weight) * self.adj_normalize(adj)).toarray())
        return eigen_adj
    ...
    def initModel(self):
        super(SEPT_social, self).initModel()
        ...
        social_ppr_mat = self.cal_ppr_social_mat()
    ...
    def trainModel(self):
        ...
        loss = loss + self.ppr_rate * self.social_ppr_loss
        ...
        for epoch in range(self.maxEpoch):
            ...
            if epoch > -1:
                ...
                for n, batch in enumerate(self.next_batch_pairwise()):
                    ...
                    _, l1, l2, l3, = self.sess.run([v2_op, rec_loss, self.neighbor_dis_loss, self.social_ppr_loss],feed_dict=feed_dict)
                    # pdb.set_trace()
