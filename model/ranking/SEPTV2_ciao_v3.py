...
'''
class SEPTV2_ciao_v3(SocialRecommender, GraphRecommender):
    def __init__(self, conf, trainingSet=None, testSet=None, relation=None, fold='[1]'):
        GraphRecommender.__init__(self, conf=conf, trainingSet=trainingSet, testSet=testSet, fold=fold)
        SocialRecommender.__init__(self, conf=conf, trainingSet=trainingSet, testSet=testSet, relation=relation,fold=fold)

    def readConfiguration(self):
        super(SEPTV2_ciao_v3, self).readConfiguration()
        args = config.OptionConf(self.config['SEPTV2_ciao_v3'])
    ...
    def initModel(self):
        super(SEPTV2_ciao_v3, self).initModel()
        ...
        #         initializer([self.emb_size, self.emb_size]), name='W_1')
        # self.weights_2 = tf.Variable(
        #         initializer([self.emb_size, self.emb_size]), name='W_2')

        
        if self.ppr_rate != 0.0:
            # add social-based ppr mat
            social_ppr_mat, social_ppr_sp_mat = self.cal_ppr_social_mat(weight=0.15)
    ...
    def trainModel(self):
        ...
        # modify inbatch users
        if self.ppr_rate != 0.0:
            # 1. add social-based ppr loss
            # too many zero, cause the result bad;
            self.user_user_sim =  tf.nn.sigmoid(tf.matmul(self.batch_user_emb, tf.transpose(self.rec_user_embeddings, perm=[1, 0])) )#(bs, user_num)
            self.batch_social_ppr_mat = tf.nn.embedding_lookup(self.social_ppr_mat, self.u_idx) #(bs, user_num)

            # sum_embeddings = tf.matmul(self.batch_user_emb, self.weights_1) 
            # tf.layers.dense(inputs=tf.reshape(self.batch_user_emb, [-1, self.emb_size]), units=self.graph_label_num, activation=None)
            
            # # cos sim
            # self.user_user_sim = tf.matmul(self.batch_user_emb, tf.transpose(self.rec_user_embeddings, perm=[1, 0])) #(bs, voc)
            # tensor1_norm = tf.reshape(tf.sqrt(tf.reduce_sum(tf.math.square(self.rec_user_embeddings), axis=-1)), [1, -1]) #(1, voc)
            # tensor2_norm = tf.reshape(tf.sqrt(tf.reduce_sum(tf.math.square(self.batch_user_emb), axis=-1)), [-1, 1]) # (bs, 1)
            # self.user_user_sim = self.user_user_sim / (tensor1_norm * tensor2_norm) #(bs, voc)
            
            # self.batch_social_ppr_mat = tf.nn.embedding_lookup(self.social_ppr_mat, self.u_idx) #(bs, user_num)
            # gamma = 2.0
            # alpha = 0.9
            # y_pred = tf.nn.sigmoid(self.user_user_sim)
            # labels = tf.to_float(self.batch_social_ppr_mat)
            # L = -labels*(1-alpha)*((1-y_pred)*gamma)*tf.log(y_pred) - (1-labels)*alpha*(y_pred**gamma)*tf.log(1-y_pred)
            # self.social_ppr_loss = self.ppr_rate * tf.reduce_mean(L)
            
            # select top k for learning
            top_k = 20
            kth_largest = tf.expand_dims(tf.math.top_k(self.batch_social_ppr_mat, top_k)[0][:, -1], 1) #(bs, 1)
            self.batch_social_ppr_mat_clip = tf.where(self.batch_social_ppr_mat < kth_largest, tf.zeros_like(self.batch_social_ppr_mat), self.batch_social_ppr_mat)
            pos_weight = (self.num_users - top_k) *1.0 / top_k
            # pdb.set_trace()
            self.social_ppr_loss = self.ppr_rate * tf.losses.mean_squared_error(self.batch_social_ppr_mat_clip, self.user_user_sim, weights=pos_weight)
            loss += self.social_ppr_loss # ppr is good;
        ...
        for epoch in range(self.maxEpoch):
            ...
            # if epoch > -1:
                ...
                for n, batch in enumerate(self.next_batch_pairwise()):
                    ...
                    # _, opt_l, l1, social_ppr_loss = self.sess.run([v2_op, loss, rec_loss, self.social_ppr_loss],feed_dict=feed_dict)
