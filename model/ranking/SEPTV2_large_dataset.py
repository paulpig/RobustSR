...
'''
class SEPTV2_large_dataset(SocialRecommender, GraphRecommender):
    def __init__(self, conf, trainingSet=None, testSet=None, relation=None, fold='[1]'):
        GraphRecommender.__init__(self, conf=conf, trainingSet=trainingSet, testSet=testSet, fold=fold)
        SocialRecommender.__init__(self, conf=conf, trainingSet=trainingSet, testSet=testSet, relation=relation,fold=fold)

    def readConfiguration(self):
        super(SEPTV2_large_dataset, self).readConfiguration()
        args = config.OptionConf(self.config['SEPTV2_large_dataset'])
    ...
    def cal_ppr_social_mat_test_tmp(self, weight=0.15, type_enc="ppr", bidirectional=False):
        def normalization(M):
            rowsum = np.array(M.sum(1))
            d_inv = np.power(rowsum, -0.5).flatten()
            d_inv[np.isinf(d_inv)] = 0.
        ...
        return social_mat.toarray(), normalization(social_mat) 
        # c = 0.15
        # if type_enc == "ppr":
        #     eigen_adj = weight * inv((sp.eye(adj.shape[0]) - (1 - weight) * self.adj_normalize(adj)).toarray()) #array format
        # elif type_enc == "hk":
    ...
    def initModel(self):
        super(SEPTV2_large_dataset, self).initModel()
        ...
        if self.ppr_rate != 0.0:
            ...
            # dir_file = "./dataset/{}/".format(self.dataset_name)
            # if not os.path.exists(dir_file):
            #     os.makedirs(dir_file)
            # file_path = dir_file+"social_ppr_mat.npy"
            # if os.path.exists(file_path):
            #     social_ppr_mat = np.load(file_path)
            #     social_ppr_sp_mat = None
            # else:
            #     social_ppr_mat, social_ppr_sp_mat = self.cal_ppr_social_mat()
            #     np.save(dir_file+"social_ppr_mat", social_ppr_mat)

            # social_ppr_mat, array.
            # social_ppr_mat, social_ppr_sp_mat = self.cal_ppr_social_mat()
            social_ppr_mat, social_ppr_sp_mat = self.cal_ppr_social_mat_test_tmp()

            self.social_ppr_mat_np = social_ppr_mat
            ...
            # social_mat = tf.sparse.to_dense(social_mat)
            self.social_ppr_mat_placeholder = tf.placeholder(dtype=tf.float32, shape=[self.num_users, self.num_users])
            ...
            # with tf.device("/gpu:{}".format(1)):
                # self.social_ppr_mat = tf.get_variable('social_ppr_mat', [self.num_users, self.num_users])
            self.social_ppr_mat = tf.Variable(self.social_ppr_mat_placeholder, trainable=False, name='social_ppr_mat')
            # social_ppr_mat_csr = sp.csr_matrix(social_ppr_mat)
    ...
    def trainModel(self):
        ...
        if self.ppr_rate != 0.0:
            ...
            self.social_ppr_loss = self.ppr_rate * tf.losses.mean_squared_error(self.batch_social_ppr_mat, self.user_user_sim)
        ...
        v1_op = v1_opt.minimize(rec_loss, colocate_gradients_with_ops=True)
        v2_opt = tf.train.AdamOptimizer(self.lRate)
        v2_op = v2_opt.minimize(loss, colocate_gradients_with_ops=True)
        init = tf.global_variables_initializer()
        # self.sess.run(init)
        self.sess.run(init, feed_dict={self.social_ppr_mat_placeholder: self.social_ppr_mat_np})
        

        # init data
        # self.sess.run(self.social_ppr_mat.assign(self.social_ppr_mat_placeholder), {self.social_ppr_mat_placeholder: self.social_ppr_mat_np})
