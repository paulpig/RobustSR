...
LightGCN model
"""
class LightGCN_PairNorm(GraphRecommender):
    def __init__(self,conf,trainingSet=None,testSet=None,fold='[1]'):
        super(LightGCN_PairNorm, self).__init__(conf,trainingSet,testSet,fold)
        args = OptionConf(self.config['LightGCN'])
        self.n_layers = int(args['-n_layer'])
        # pdb.set_trace()

    def initModel(self):
        super(LightGCN_PairNorm, self).initModel()
        #SEPT对齐, 修正初始化.
        ...
        for k in range(self.n_layers):
            ...
            x = x - col_mean
            rownorm_mean = tf.reduce_sum(tf.math.pow(x, tf.constant(2.0, dtype=tf.float32)), axis=1)
            rownorm_mean = tf.reduce_mean(rownorm_mean)
            rownorm_mean = tf.math.sqrt(rownorm_mean + 1e-6)
            # rownorm_mean = (1e-6 + x.pow(2).sum(axis=1).mean()).sqrt()
            x = self.scale * x / rownorm_mean
            # pdb.set_trace()
            # 3.add relu
            ego_embeddings = tf.nn.relu(x)
