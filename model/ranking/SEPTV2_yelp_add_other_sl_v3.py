...
'''
class SEPTV2_yelp_add_other_sl_v3(SocialRecommender, GraphRecommender):
    def __init__(self, conf, trainingSet=None, testSet=None, relation=None, fold='[1]'):
        GraphRecommender.__init__(self, conf=conf, trainingSet=trainingSet, testSet=testSet, fold=fold)
        SocialRecommender.__init__(self, conf=conf, trainingSet=trainingSet, testSet=testSet, relation=relation,fold=fold)

    def readConfiguration(self):
        super(SEPTV2_yelp_add_other_sl_v3, self).readConfiguration()
        args = config.OptionConf(self.config['SEPTV2_yelp_add_other_sl_v3'])
    ...
    def hierarchical_self_supervision_new_v3(self, em, edge_embeddings, add_global=True, add_local=True):
        def row_shuffle(embedding):
            return tf.gather(embedding, tf.random.shuffle(tf.range(tf.shape(embedding)[0])))
        def row_column_shuffle(embedding):
            corrupted_embedding = tf.transpose(tf.gather(tf.transpose(embedding), tf.random.shuffle(tf.range(tf.shape(tf.transpose(embedding))[0])))) #(user, dim)
    ...
    def initModel(self):
        super(SEPTV2_yelp_add_other_sl_v3, self).initModel()
        ...
        if self.inter_cl_w != 0.0:
            # social_ppr_sp_mat_true = sp.csr_matrix(social_ppr_mat)
            ...
            # self.sl_loss = self.hierarchical_self_supervision_new(self.rec_user_embeddings, social_ppr_sp_mat_true, add_local=True, add_global=True)
            # self.sl_loss = self.hierarchical_self_supervision_new(self.rec_user_embeddings, social_ppr_sp_mat_true, add_local=True, add_global=False)
            # self.sl_loss = self.hierarchical_self_supervision_new(self.rec_user_embeddings, social_ppr_sp_mat_true, add_local=False, add_global=True)
            self.social_ppr_cluster_emb = self.sampleTopkUsersKeepOri(self.rec_user_embeddings, top_k=self.top_k, social_ppr_mat=social_ppr_mat, mask_ori=True, add_norm=True, add_self=True, social_layer_num=2) #目前的sota.

            self.sl_loss = self.hierarchical_self_supervision_new_v3(self.rec_user_embeddings, self.social_ppr_cluster_emb, add_local=True, add_global=True)
        ...
        for epoch in range(self.maxEpoch):
            ...
            # if epoch > -1:
                ...
                for n, batch in enumerate(self.next_batch_pairwise()):
                    ...
                    _, opt_l, l1, l2, l4, l5 = self.sess.run([v2_op, loss, rec_loss, self.social_ppr_loss, self.sl_loss, reg_loss],feed_dict=feed_dict)
